...
const loginUser = async (e: Event) => {
  supabase.auth.signIn({ email: userCredentials.email, password: userCredentials.password })
    .then((res) => {
      console.log(res)
    }).catch((err) => {
      console.error(err)
...
<template>
  <form class="flex flex-col items-center justify-evenly h-fit w-1/3" @submit="(e) => loginUser(e)">
